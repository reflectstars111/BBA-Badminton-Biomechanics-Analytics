...
from badminton_data_process.core.schemas import REQUIRED_MATCH_FIELDS
...
def validate_matches(csv_path: Path) -> tuple[int, list[dict[str, str]], list[str]]:
    ...
    for idx, row in enumerate(rows, start=1):
        missing = [key for key in REQUIRED_MATCH_FIELDS if not row.get(key)]
        if missing:
            missing_count += 1
    ...
    if duplicated_match_ids:
        errors.append("Duplicated match_id values: " + ", ".join(sorted(duplicated_match_ids)))

    if missing_count and not errors:
        errors.append(f"Rows with missing required fields: {missing_count}")
    return (0 if not errors else 2), rows, errors


def print_validation(exit_code: int, rows: list[dict[str, str]], errors: list[str]) -> None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    exit_code, rows, errors = validate_matches(args.csv)
    print_validation(exit_code, rows, errors)
    if args.summary:
        print_summary(rows)
    return exit_code
