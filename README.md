...
>
> 上游是一个「逐帧检测 + 轨迹可视化」的演示工具；本项目在其计算机视觉能力之上，
...
| 维度 | 上游 yo-WASSUP | 本项目 |
| --- | --- | --- |
| **输入** | 干净片段 + 手动标场地 | 未剪辑完整转播，自动清洗 |
| **回合切分** | 无（连续球场画面） | Main View 门禁 + Usable Rally 切分，自动剔除采访/回放/特写/计分牌/切视角 |
| **球场标定** | 取最高分候选，易被绿色场地面/广告边缘误导 | 白线 Hough 检测 + 13 条规则线支持率 + 几何/重投影/凸性/时序稳定性多帧验证，证据不足**明确拒绝而非猜测** |
| **羽毛球检测** | 单帧 YOLO，稀疏轨迹 | TrackNet 多帧深度检测，约 93% 密集可见轨迹 |
| **球员定位** | 单点投影，空中点会引入系统偏差 | RTMPose 姿态 + **双锚点**（躯干中心 / 地面接触点分离），正式米制坐标只来自地面接触点 |
| **失败语义** | 「跑完 = 成功」 | missing / rejected / failed / empty / success 五态分离，**缺失不伪装成有效、失败不伪装成成功** |
| **可复现** | 输出目录拼接、无指纹 | Run Manifest 记录配置/输入/模型指纹 + 每阶段输入输出与耗时，支持断点 resume |
| **战术分析** | 无 | 击球/落点（物理规则：每拍羽毛球落地恰好一次）、跑动距离、覆盖面积、站位区域占比，每项指标带资格门控 |
| **工程硬化** | — | 122 项测试；CUDA 加速（RTMPose 人物阶段 CPU ~603s → CUDA ~41s）；H.264 浏览器兼容导出 |
